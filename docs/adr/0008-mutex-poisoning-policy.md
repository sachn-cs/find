...
1. **Cross-batch match coordination** in [`src/search.rs::precompute_chunk`](../../src/search.rs): a `Mutex<Option<SearchMatch>>` shared across Rayon workers. The lock is held briefly to check for a match and to store a match.
2. **File-handle serialization** in [`src/persistence.rs::FileCacheWriter`](../../src/persistence.rs): a `Mutex<File>` that serializes `seek + write_all` on platforms without `pwrite_at` (non-Unix).
...
For the cross-batch `match_found` mutex in `precompute_chunk`:
...
The `into_inner()` call at the end of `precompute_chunk` explicitly handles poisoning:
...
For the file-handle mutex in `FileCacheWriter`:
...
- [`src/search.rs::precompute_chunk`](../../src/search.rs) — Policy A implementation
- [`src/persistence.rs::FileCacheWriter`](../../src/persistence.rs) — Policy B implementation
